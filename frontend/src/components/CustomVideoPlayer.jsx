...
const CustomVideoPlayer = forwardRef(
  ({ src, subtitleSrc, movie, onDurationChange }, ref) => {
    ...
    // 🎬 Prosljeđivanje videoRef-a parent komponenti (za ThumbnailSelector)
    useImperativeHandle(
      ref,
      () => ({
    ...
    useEffect(() => {
      if (movie?.id && videoRef.current) {
        ...
        if (
          progress &&
          progress.watchedPercentage < 95 &&
          progress.timestamp > 10
        ) {
    ...
    useEffect(() => {
      ...
      const updateDuration = () => {
        ...
        setIsLoading(false);
        // Obavijesti parent komponent o trajanju videa
        if (onDurationChange) {
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 z-10">
            ...
            <div className="relative mb-6">
              <div className="w-20 h-20 border-4 border-accent/30 rounded-full animate-spin">
                <div
                  className="absolute top-1/2 left-1/2 w-10 h-10 -mt-5 -ml-5 border-4 border-t-accent border-r-transparent border-b-transparent border-l-transparent rounded-full animate-spin"
                  style={{
              ...
              ></div>
              <div
                className="absolute -bottom-1 -left-1 w-3 h-3 bg-accent rounded-full animate-pulse"
                style={{ animationDelay: "0.4s" }}
              ></div>
              <div
                className="absolute -bottom-1 -right-1 w-3 h-3 bg-accent rounded-full animate-pulse"
